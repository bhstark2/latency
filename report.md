...
In equipment that can support a variety of link rates (such as DSL modems, cable modems, and WiFi gear) statically sized FIFO buffers introduce even more buffering delay when the link is operating at a slower rate. FIFOs that can be adjusted to a more appropriate size for the link operating rate can improve the latency performance (in particular when the link is under load), but making the buffer too small will impact the throughput of congestion controlled traffic.
