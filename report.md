...
Evaluating network user QoE for latency should begin by defining an appropriate service level objective (SLO). The objective(s) can be based on known application requirements, or on measured application performance resulting from latency changes in a lab environment. A public internet service must satisfy an extremely wide range of applications and use cases, and absent a well-defined, lab-based SLO, network providers can apply the following indirect SLO definition method.

Human users perform tasks using applications running over a system of networks. The performance of the entire system contributes to the user experience. The users’ applications make demands on the network system. The most prevalent of these demands are the demand for bandwidth to move payload, and the demand for acceptable latency to accommodate turns. Payload includes everything a user’s device will process or render. It can also include everything the server will process or store on behalf of the user. Turns are packet or message exchanges that do not move payload. Turns are gatekeepers to the payload transfers.

Elapsed time to transfer payload is primarily governed by bandwidth, which is not a focus of this report. However, it is also governed by latency, due to the inherent ways protocols such as TCP assure proper payload delivery. This is discussed in another section (see xx).

Much of what users refer to as “responsiveness” is the lag time generated by the product of turns and latency. The number of turns required for an interaction is generally rising. The number of turns in typical web pages has been growing exponentially since the dawn of the web browser. Given that lag is a product of turns and latency, minimizing latency is essential.

In general, we cannot know the user and application in use, nor the number of turns being generated. However, network providers can rely upon the assumption that most users are generally satisfied with performance most of the time. If not, they complain. Assuming that complaints are minimal, the network is performing “as expected,” and most users are satisfied. We can use this observation to define satisfactory latency that constitutes a baseline.

Given these ground rules, it is recommended that network service providers measure latency using a simple RTT ping method. These measurements should be made often and with random start times for statistically sound sampling. They should also be generated across many client-target pairs, where targets represent general user-destination locations. Pairs spanning only the last mile are not long enough to meaningfully represent the network paths users actually traverse. The pairs should crisscross the network in logical and consistent network distance groupings. The system of test pairs becomes a latency-based network performance telemetry system.

Latency measurement results should be placed in datasets by groups of interest (popular sites, geographic zones, daily, hourly, etc.) and reported as medians. Medians are recommended because they are the most informative of the summary statistics proposed in this report. Ping loss should also be included in the dataset. Current measurement systems typically remove lost pings from the results because they have no time value (millisecond). However, from a telemetry viewpoint, this is valuable information about the underlying network behavior because it reflects data that never arrived (i.e., infinite latency).

This approach needs to find “best case” time periods which will show median values that become the baselines for each reporting group. These can be determined when the network is known to be underutilized. Baseline latency is, therefore, close to idle latency for any given path.

Production performance measurements report working latency, which includes effects of cross-traffic, queueing delays, system load, rerouting, etc. All of these latency impairments are exactly what impacts user packets. The basic telemetry system does not know why latency was degraded, but can reliably report that the user experience was degraded. Degradation causes can be determined by other measurements outside the scope of this report. This system can inform a service provider with a few investigation starting points such as incident date/time, users impacted (client group), destinations impacted (target group), and provide a general geographic description of the network path.

QoE per reporting group is the change of measured median relative to the baseline for that group. The change can be converted into a score that can be tracked over time. A “non-compliant” threshold is defined by a score, which if exceeded, represents non-satisfactory latency performance. If latency is found to be excessive, changes can be made in the network to bring it back into compliance.

It is important to maintain long-term stability within the latency measurement system. Once the test client-target pairs and their baselines are established, changes should be minimal so the system can supply meaningful measures of performance trends over time (at least several months). When changes are needed—for example targets are no longer available—new pairs should be established to closely match the retired pair.

Two thresholds in this approach bin the latency values. Using median values bins test results into two buckets, below and above the 50th percentile. The resulting median values can be graded as above the baseline or a specific SLO non-compliance point relative to the baseline. This approach is sensitive to changes showing true performance gradients.

There is a different summary approach, called Apdex [https://www.apdex.org/] , which is an open standard. Apdex is an improved median approach which uses three bins. They are latency values where the user is experiencing satisfied, tolerating, or frustrated responsiveness. It also uses baselining to set the thresholds between the bins. Apdex explicitly evaluates the effects of the long-tail values found in most latency measurement results. Finally, it also uses a non-compliant grading of the Apdex scores. This is a more sensitive approach to analyzing and reporting latency values. See “US ISP Long-term Latency Benchmark Trends” report as an example of Apdex used to track QoE across the US [https://www.netforecast.com/netforecast-report-analyzes-isp-latency-patterns-from-march-2020-through-march-2021/].

An important distinction between using medians and Apdex is the reporting scales. Median values are on a scale from zero to infinity with a unique range for each measured path. These multiple unbounded ranges pose a problem for evaluating performance in terms of a system-wide SLO. The Apdex methodology generates results that are normalized into a simple zero (all samples unsatisfactory) to 1 (all satisfactory) scale. Low scores indicate poor performance. It is easier to pick a point on this scale which represents the QoE SLO non-compliance point. It also permits merging results for different paths since the scores can be made distance neutral.

Regardless of the summary approach, individual client-target pair measurements must be aggregated into larger sample groups for statistically significant results and meaningful reporting. There is a natural tendency to continue aggregating results to the point where a report covers all locations (pairs) over a large time horizon (often a month), but network service providers should avoid focusing on broad views at the expense of narrow views. Most network performance incidents occur in small pockets of a network and impact small, but often vocal, groups of users. Therefore, the reporting system should be automated to find and highlight incidents at fine granularity. This is an area well served by advanced algorithms that correlate small events into more useful observations.
